...
public class ImageUtil {
    ...
    // 图片默认缩放比率
    private static final double DEFAULT_SCALE = 3.0d;
...
//    private static final String SUFFIX = "-thumbnail";
     ...
     * @throws IOException
     */
    public static List<String> generateThumbnail2DirectoryWatermark(double scale, File waterPic, String pathname, String... files) throws IOException {
        ...
        Thumbnails.of(files)
                // 图片缩放率，不能和size()一起使用
                .scale(scale).watermark(Positions.CENTER, ImageIO.read(waterPic), 0.5f)
                .outputQuality(0.8f)
                // 缩略图保存目录,该目录需存在，否则报错
                .toFiles(new File(pathname), Rename.NO_CHANGE);
        List<String> list = new ArrayList<>(files.length);
    ...
    public static void watermark(String fromPic, File waterPic, String toPic) throws IOException {
        //给图片加水印，watermark(位置，水印图，透明度)Positions.CENTER表示加在中间
        Thumbnails.of(fromPic).watermark(Positions.CENTER, ImageIO.read(waterPic), 0.5f)
